/* Athena Docs — Header enhancements
   - Injects version chip after site title (live from GitHub tags)
   - Injects GitHub repo + stars widget (Prescott-Data/athena ★ N)
...
function initHeader() {
  ...
  if (chip) {
    fetch('https://api.github.com/repos/Prescott-Data/athena/tags')
  ...
  if (inner && palette && !existingGhBtn) {
    ...
    ghBtn.href = 'https://github.com/Prescott-Data/athena';
    ghBtn.target = '_blank';
    ghBtn.rel = 'noopener noreferrer';
    ghBtn.className = 'nx-gh-btn';
    ghBtn.setAttribute('aria-label', 'Star Athena on GitHub');
    ghBtn.innerHTML =
      '<span class="nx-gh-repo">Prescott-Data/athena</span>' +
  ...
  /* ── 3. Async star count ────────────────────────────── */
  fetch('https://api.github.com/repos/Prescott-Data/athena')
    .then(function (r) { return r.json(); })
    .then(function (data) {
